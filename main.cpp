...
int main(int argc, char* argv[]) {
    ...
    AppTab   activeTab = TAB_CODE;
    SDL_Rect tabRects[TAB_COUNT];
    ...
    palette.catBarX = 0;
    palette.catBarY = TAB_BAR_H;          // ← offset by tab bar
    palette.catBarW = CAT_ICON_W;
    palette.catBarH = SCREEN_HEIGHT - TAB_BAR_H;
    palette.blockListX = CAT_ICON_W;
    palette.blockListY = TAB_BAR_H;       // ← offset by tab bar
    palette.blockListW = BLOCK_LIST_W;
    palette.blockListH = SCREEN_HEIGHT - TAB_BAR_H;
    ...
    while (!quit) {
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
                ...
                if (my < TAB_BAR_H) {
                    for (int i = 0; i < TAB_COUNT; i++) {
                        if (mx >= tabRects[i].x && mx < tabRects[i].x + tabRects[i].w) {
                            activeTab = (AppTab)i;
                            break;
                        }
                    }
            ...
            if (activeTab == TAB_COSTUMES) {
                ...
                if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
                    int mx = e.button.x, my = e.button.y;
                    if (mx < PALETTE_WIDTH && my >= TAB_BAR_H) {
                        int itemH = COSTUME_THUMB + 24 + 4;
                        int relY  = my - TAB_BAR_H - 40 - costumePanel.scrollOffset;
                        if (relY >= 0) {
                            int idx = relY / itemH;
            ...
            if (varsPanel.creating && e.type == SDL_KEYDOWN) {
                if (e.key.keysym.sym == SDLK_RETURN || e.key.keysym.sym == SDLK_KP_ENTER) {
                    if (!varsPanel.newVarName.empty()) {
                        Variable nv; nv.name = varsPanel.newVarName; nv.value = 0.0f; nv.showOnStage = true;
                    ...
                    varsPanel.creating = false; varsPanel.newVarName = ""; SDL_StopTextInput();
            ...
            if (varsPanel.creating && e.type == SDL_TEXTINPUT) {
                varsPanel.newVarName += e.text.text; continue;
            }
            ...
            if (myBlocksCreating && e.type == SDL_KEYDOWN) {
                if (e.key.keysym.sym == SDLK_RETURN || e.key.keysym.sym == SDLK_KP_ENTER) {
                    ...
                    myBlocksCreating = false; newBlockName = ""; SDL_StopTextInput();
                } else if (e.key.keysym.sym == SDLK_ESCAPE) {
                    myBlocksCreating = false; newBlockName = ""; SDL_StopTextInput();
            ...
            if (myBlocksCreating && e.type == SDL_TEXTINPUT) {
                newBlockName += e.text.text; continue;
            }
            ...
            if (e.type == SDL_MOUSEWHEEL) {
                ...
                if (activeTab == TAB_CODE &&
                    point_in_rect(mx, my, palette.blockListX, palette.blockListY,
            ...
            if (activeTab == TAB_CODE) {
                if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
                    ...
                    if (activeInput) {
                        activeInput->editing = false; activeInput = nullptr; SDL_StopTextInput();
                    }
                    ...
                    if (point_in_rect(mx, my, playBtn.x, playBtn.y, playBtn.w, playBtn.h)) {
                        ...
                        varsPanel.creating = true; varsPanel.newVarName = ""; SDL_StartTextInput();
                        continue;
                    } else if (isMyBlocks && makeBlockBtn.w > 0 &&
                               point_in_rect(mx, my, makeBlockBtn.x, makeBlockBtn.y,
                                             makeBlockBtn.w, makeBlockBtn.h)) {
                        myBlocksCreating = true; newBlockName = ""; SDL_StartTextInput();
                        continue;
        ...
        draw_tab_bar(renderer, fontSmall, activeTab, tabRects);
        ...
        if (activeTab == TAB_CODE) {
            ...
            if (varsPanel.creating)   draw_name_dialog("New Variable Name:", varsPanel.newVarName);
            if (myBlocksCreating)     draw_name_dialog("Block Name:", newBlockName);

        } else if (activeTab == TAB_COSTUMES) {
            // Show full-panel costume list
            CostumePanel fullPanel;
            fullPanel.x = 0;
            fullPanel.y = TAB_BAR_H;
            fullPanel.w = PALETTE_WIDTH;
            fullPanel.h = SCREEN_HEIGHT - TAB_BAR_H;
            fullPanel.scrollOffset  = costumePanel.scrollOffset;
            fullPanel.selectedIndex = costumePanel.selectedIndex;
            draw_costume_panel(renderer, fontSmall, fontBig, fullPanel, &sprite);
